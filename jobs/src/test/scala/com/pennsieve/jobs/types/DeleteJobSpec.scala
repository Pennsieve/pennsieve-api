...
import com.pennsieve.domain.{ CoreError, StorageAggregation, ThrowableError }
...
class DeleteJobSpec
  ...
  it should "soft delete descendants and finalize the root package" in { _ =>
    val user = createUser(email = "deleter@test.com")
    val dataset = createDataset(user = user)
    val root = createPackage(
      user = user,
      dataset = dataset,
      state = PackageState.DELETING
    )
    val child =
      createPackage(user = user, dataset = dataset, parent = Some(root))
    val grandChild = createPackage(
      user = user,
      dataset = dataset,
      parent = Some(child),
      `type` = Slide
    ...
    val rootAfter = database.run(packageTable.get(root.id).result.head).await
    rootAfter.state should be(PackageState.DELETED)

    val childAfter = database.run(packageTable.get(child.id).result.head).await
    childAfter.state should be(PackageState.DELETING)
    childAfter.name should startWith("__DELETED__")

    val grandChildAfter =
      database.run(packageTable.get(grandChild.id).result.head).await
    grandChildAfter.state should be(PackageState.DELETING)
    grandChildAfter.name should startWith("__DELETED__")
  }

  it should "decrement storage once when the deleted package was READY" in {
    _ =>
      val user = createUser(email = "deleter@test.com")
      val dataset = createDataset(user = user)
      val parent = createPackage(user = user, dataset = dataset)
      ...
      val sm = storageManager(testOrganization)
      sm.incrementStorage(spackages, 1000, pkg.id).await

      def storageOf(agg: StorageAggregation, id: Int): Option[Long] =
        sm.getStorage(agg, List(id)).await.value.get(id).flatten
      ...
      storageOf(spackages, pkg.id) should be(Some(0))
      storageOf(spackages, parent.id) should be(Some(0))
      storageOf(sdatasets, dataset.id) should be(Some(0))
      storageOf(sorganizations, testOrganization.id) should be(Some(0))

      // a re-delivered message must not decrement storage a second time
      assert(deleteJob.creditDeleteJob(msg).await.isRight)
  ...
  it should "not decrement storage for legacy messages without originalState" in {
    _ =>
      val user = createUser(email = "deleter@test.com")
      val dataset = createDataset(user = user)
      val pkg = createPackage(
