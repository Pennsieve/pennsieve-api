...
resource "aws_ssm_parameter" "data_postgres_user" {
  ...
  type  = "String"
  value = var.data_postgres_user
}

resource "aws_ssm_parameter" "data_postgres_use_ssl" {
  name  = "/${var.environment_name}/${var.service_name}/data-postgres-use-ssl"
  type  = "String"
  value = var.data_postgres_use_ssl
}